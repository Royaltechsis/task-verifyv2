    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="page-container pt-32 pb-12 max-w-4xl">
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-3xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-black text-navy-900">Account Settings</h2>
          ...
          <div className="bg-white rounded-[2rem] border border-slate-100 shadow-xl overflow-hidden mb-8">
            ...
            {user?.role === 'worker' && workerProfile && (
              <div className="p-8 space-y-8">
                ...
                {!isEditing && (
                  <Link to="/trust-score-breakdown" className="block group">
                    <div className="bg-navy-950 text-white rounded-2xl p-6 relative overflow-hidden group-hover:scale-[1.02] transition-all">
                      <div className="absolute top-0 right-0 opacity-10 w-32 h-32 transform translate-x-8 -translate-y-8 group-hover:rotate-12 transition-transform">
                      ...
                      <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-6 group-hover:text-white transition-colors">
                ...
                {kycStatus?.status !== 'approved' && !isEditing && (
                  <Button 
                    onClick={() => setShowKYC(true)}
                    className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-black text-xs uppercase tracking-widest px-6 h-14 rounded-xl"
                  >
                    Complete Identity KYC <ShieldCheck className="ml-2 w-4 h-4 text-white" />
                  </Button>
                )}
            ...
            {user?.role === 'buyer' && (
              <div className="p-8">
                <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 text-center">
                   ...
                   <p className="text-slate-500 font-medium">You are logged in as a Buyer. Use the Wallet Hub to manage your Squad Escrow transactions and fund your wallet.</p>
                </div>
              </div>
            )}
          </div>
