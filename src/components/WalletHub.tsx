...
import { FullPageLoader } from './SkeletonLoader'
...
export default function WalletHub() {
  ...
  const isEmployer = user?.role?.toLowerCase() === 'buyer' || user?.role?.toLowerCase() === 'employer'
      ...
      {isEmployer ? (
        ...
        <div className="grid lg:grid-cols-3 gap-8">
            ...
            <Card className="lg:col-span-1 border-none shadow-2xl shadow-navy-100/50 rounded-[2.5rem] p-8 bg-white flex flex-col items-center">
              <Link to="/trust-score-breakdown" className="text-center mb-8 group cursor-pointer">
                <h2 className="font-black text-navy-900 text-xl tracking-tight group-hover:text-emerald-600 transition-colors">economic identity layer Trust Score</h2>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1 group-hover:text-emerald-500 transition-colors">AI-Driven Financial Reputation</p>
              </Link>
              ...
              <Link to="/trust-score-breakdown" className="mt-5 w-full rounded-[1.5rem] bg-white border border-slate-100 p-5 shadow-sm hover:shadow-md transition-all group">
                <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3 group-hover:text-emerald-500">Trust Snapshot</div>
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="text-4xl font-black text-navy-900 leading-none group-hover:text-emerald-600 transition-colors">{trustScore}</div>
              ...
              <Link to="/trust-score-breakdown" className="mt-8 text-xs font-black uppercase tracking-widest text-slate-400 hover:text-navy-900 gap-2 flex items-center justify-center">
            ...
            <Card className="lg:col-span-3 border-none shadow-2xl shadow-navy-100/50 rounded-[2.5rem] p-10 bg-white">
              <div className="flex items-center gap-3 mb-10">
                ...
                <h3 className="font-black text-navy-900 text-xl tracking-tight">Financial Intelligence</h3>
              </div>
